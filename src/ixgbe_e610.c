			   ...
			   void *buf, u16 buf_size)
{
	...
	do {
		...
		if (valid_buf && (desc->flags &
			...
			for (i = 0; i < tmp_buf_size / 4; i++) {
				tmp_buf[i] = IXGBE_R32_Q(hw, PF_HIBA(i));
			...
			const char *prefix)
{
	...
	case IXGBE_ACI_CAPS_NEXT_CLUSTER_ID:
		...
		hw_dbg(hw, "%s: next_cluster_id_support = %d\n",
...
/**
 * ixgbe_aci_nvm_update_empr - NVM update EMPR
 * @hw: pointer to the HW struct
 *
 * Update empr using ACI command (0x0709). This command allows SW to
 * request an EMPR to activate new FW.
 *
 * Return: the exit code of the operation.
 */
s32 ixgbe_aci_nvm_update_empr(struct ixgbe_hw *hw)
{
	struct ixgbe_aci_desc desc;
 ...
 * Return: the exit code of the operation.
 */
s32 ixgbe_nvm_set_pkg_data(struct ixgbe_hw *hw, bool del_pkg_data_flag,
			   u8 *data, u16 length)
{
 ...
 * Return: the exit code of the operation.
 */
s32 ixgbe_nvm_pass_component_tbl(struct ixgbe_hw *hw, u8 *data, u16 length,
			...
			struct ixgbe_nvm_access_data *data)
{
	...
	hw_dbg(hw, "NVM access: reading register %08x\n", cmd->offset);
			...
			struct ixgbe_nvm_access_data *data)
{
	...
	hw_dbg(hw, "NVM access: writing register %08x with value %08x\n",
